# LLMOps 이직 포트폴리오 로드맵 (v0.1 당시 계획 기록)

> 이 문서는 구현 전 계획을 보존한 기록이다. 아래 Milestone 1~5는 v0.2에서
> 완료됐으며, 현재 상태와 검증 증거는 루트 `README.md`와
> `docs/final-review/final-release-review.json`을 기준으로 한다.
